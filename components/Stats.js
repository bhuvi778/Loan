...
export default function Stats() {
  ...
  return (
    <section id="stats" className="relative py-8 lg:py-12 bg-black overflow-hidden">
      {/* Background */}
      <div className="absolute inset-0 bg-gradient-to-b from-black via-[#000a14] to-black" />
      <div
        className="absolute inset-0"
        style={{
          backgroundImage:
            "linear-gradient(rgba(26,155,220,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(26,155,220,0.04) 1px, transparent 1px)",
          backgroundSize: "50px 50px",
        }}
      />
      <div className="orb w-[600px] h-[200px] bg-[#1a9bdc] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-[0.07] rounded-full" />
        ...
        >
          {stats.map((stat, i) => (
            <motion.div
              ...
              className="group relative text-center p-6 rounded-2xl bg-[#111111] border border-white/[0.05] hover:border-[#1a9bdc]/30 card-hover overflow-hidden"
            >
              {/* Glow on hover */}
              ...
              <p className="relative text-3xl sm:text-4xl font-black text-[#1a9bdc] counter-glow mb-2">
                <CountUp
                  end={stat.value}
                  prefix={stat.prefix}
                  suffix={stat.suffix}
              ...
              {/* Bottom accent line */}
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 w-0 group-hover:w-3/4 bg-gradient-to-r from-transparent via-[#1a9bdc] to-transparent transition-all duration-500 rounded-full" />
            </motion.div>
          ))}
        </motion.div>
